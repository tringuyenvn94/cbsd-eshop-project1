...
<head>
    ...
    <style>
     ...
     <script>
         ...
         function backToAdminHistoryByUser(){
             window.location=('adminHistoryByUsers.jsp?uid=${param.id}')
         }
         function backToAdminHistoryByDate(){
             window.location=('adminHistoryByDate.jsp?confirmDate=${param.date}')
         }
     </script>
</head>
<body>
...
<c:choose>
    ...
    <div id="main_content">
        ...
        <div class="center_content">
            ...
            <div class="center_title_bar">History Detail</div>

            <sql:query dataSource="${snapshot}" var="result">
                SELECT * from cart_product where cid=${param.cid};
            </sql:query>
            <h3>&nbsp;</h3>
            ...
            <c:choose>
                <c:when test="${empty param.from}">
                    <input type="button" value="Back" onclick="openHistoryPage();">
                </c:when>
                <c:when test="${not empty param.from}">
                    <c:choose>
                        <c:when test="${param.from =='adminUser'}">
                            <input type="button" value="Back" onclick="backToAdminHistoryByUser();">
                        </c:when>
                        <c:when test="${param.from =='adminDate'}">
                            <input type="button" value="Back" onclick="backToAdminHistoryByDate();">
                        </c:when>
                    </c:choose>
