...
<head>
    ...
    <script>
        function backToHistoryByUsers(){
            window.location=('manageTable.jsp?type=History By Users')
        }
    </script>
</head>
<body>
    ...
    <div id="main_content">
        ...
        <div class="center_content">
            ...
            <sql:query dataSource="${snapshot}" var="resultSet">
                SELECT uname,surname from user where id=${param.uid};
            </sql:query>
            <h3>&nbsp;</h3>
            <c:forEach var="user" items="${resultSet.rows}" varStatus="status">
            <h3>&nbsp;&nbsp;&nbsp;&nbsp;Customer Name: ${user.uname}&nbsp;&nbsp;${user.surname}</h3>
            </c:forEach>
                <sql:query dataSource="${snapshot}" var="result">
                    SELECT * from cart where uid=${param.uid} order by confirmDate asc;
                </sql:query>
                    ...
                    <tr>
                        ...
                        <c:choose>
                            <c:when test="${cart.confirmStatus == 1}">
                                ...
                                <td colspan="4"><a href="historyDetail.jsp?cid=${cart.cid}&date=${cart.confirmDate}&from=adminUser&id=${param.uid}" >${cart.confirmDate}</a> </td>
                            </c:when>
                            <c:when test="${cart.confirmStatus == 0}">
                                <td colspan="4">-</td>
                                <td colspan="4"><a href="historyDetail.jsp?cid=${cart.cid}&date=${cart.confirmDate}&from=adminUser&id=${param.uid}" >-</a></td>
                            </c:when>
                        </c:choose>
            ...
            <input type="button" value="Back" onclick="backToHistoryByUsers();">
